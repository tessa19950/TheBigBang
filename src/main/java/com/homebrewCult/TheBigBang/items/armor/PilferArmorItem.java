...
public class PilferArmorItem extends BigBangArmorItem {
	
	public PilferArmorItem(IArmorMaterial materialIn, EquipmentSlotType slot, int defaultColorIn, Properties builder) {
		super(materialIn, slot, defaultColorIn, builder);
	}

	@Override
	protected void applyArmorSetEffect(PlayerEntity player) {
		player.addPotionEffect(new EffectInstance(ModEffects.THIEF_EFFECT.get(),
				1728000, 1, false, false));
	}

	@Override
	protected void removeArmorSetEffect(PlayerEntity player) {
		player.removePotionEffect(ModEffects.THIEF_EFFECT.get());
	}

	@Override
	protected Set<Item> getArmorSet() {
		return new ImmutableSet.Builder<Item>().add(
				ModItems.PILFER_HELMET.getItem(),
				ModItems.PILFER_CHESTPLATE.getItem(),
				ModItems.PILFER_LEGGINGS.getItem(),
				ModItems.PILFER_BOOTS.getItem()
		).build();
	}
	...
	public <A extends BipedModel<?>> A getArmorModel(LivingEntity entityLiving, ItemStack itemStack, EquipmentSlotType armorSlot, A _default) {
		PilferArmorModel model = PilferArmorModel.INSTANCE;
