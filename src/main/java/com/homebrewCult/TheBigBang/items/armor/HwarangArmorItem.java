...
public class HwarangArmorItem extends BigBangArmorItem {

	public HwarangArmorItem(IArmorMaterial materialIn, EquipmentSlotType slot, int defaultColorIn, Properties builder) {
		super(materialIn, slot, defaultColorIn, builder);
	}

	@Override
	protected void applyArmorSetEffect(PlayerEntity player) {
		player.addPotionEffect(new EffectInstance(ModEffects.WARRIOR_EFFECT.get(),
				1728000, 0, false, false));
	}

	@Override
	protected void removeArmorSetEffect(PlayerEntity player) {
		player.removePotionEffect(ModEffects.WARRIOR_EFFECT.get());
	}

	@Override
	protected Set<Item> getArmorSet() {
		return new ImmutableSet.Builder<Item>().add(
				ModItems.HWARANG_HELMET.getItem(),
				ModItems.HWARANG_CHESTPLATE.getItem(),
				ModItems.HWARANG_LEGGINGS.getItem(),
				ModItems.HWARANG_BOOTS.getItem()
		).build();
	}
	...
	public <A extends BipedModel<?>> A getArmorModel(LivingEntity entityLiving, ItemStack itemStack, EquipmentSlotType armorSlot, A _default) {
		HwarangArmorModel model = HwarangArmorModel.INSTANCE;
