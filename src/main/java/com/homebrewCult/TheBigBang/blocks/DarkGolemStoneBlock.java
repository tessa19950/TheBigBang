...
public class DarkGolemStoneBlock extends Block {

	public static final DirectionProperty FACING = HorizontalBlock.HORIZONTAL_FACING;
	
	public DarkGolemStoneBlock(Properties builder) {
		super(builder);
	        	...
	        	for(int i = 0; i < 4; i++) {
	        		...
	        		if(otherBlock == Blocks.GRASS_BLOCK || otherBlock == ModBlocks.GRASSY_GOLEM_STONE || otherBlock == ModBlocks.GRASSY_DARK_GOLEM_STONE) {
	        			if(canSpreadTo(state, worldIn, pos)) {
	        				worldIn.setBlockState(pos, ModBlocks.GRASSY_DARK_GOLEM_STONE.getDefaultState().with(FACING, worldIn.getBlockState(pos).get(FACING)));
	        			}
	        		}
	        	}
	        }
